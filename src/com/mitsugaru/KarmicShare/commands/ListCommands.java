...
import com.mitsugaru.KarmicShare.config.KarmaInfo;
...
class ListCommands{
   ...
   private static void listMultipliers(CommandSender sender, int pageAdjust){
      ...
      Object[] array = KarmaConfig.VALUES.entrySet().toArray();
      ...
      if(valid){
         ...
         sender.sendMessage(ChatColor.BLUE + "===" + ChatColor.GOLD
               + "Karma Multiplier" + ChatColor.BLUE + "===" + ChatColor.GREEN
               + "Page: "
         ...
         for(int i = ((Karma.multiPage.get(sender.getName()).intValue()) * RootConfig
            ...
            if(i < array.length){
               ...
               String out = ChatColor.WHITE
                     ...
                     + ((Map.Entry<Item, KarmaInfo>) array[i]).getKey().name
                     + ChatColor.WHITE
                     + " Karma: "
                     + ChatColor.GOLD
                     + ((Map.Entry<Item, KarmaInfo>) array[i]).getValue()
                           .getGiveValue()
                     + " - "
                     + ((Map.Entry<Item, KarmaInfo>) array[i]).getValue()
                           .getTakeValue()
                     ...
                     + ((Map.Entry<Item, KarmaInfo>) array[i]).getKey().getId()
                     + ChatColor.WHITE
                     + " Data: "
                     + ChatColor.LIGHT_PURPLE
                     + ((Map.Entry<Item, KarmaInfo>) array[i]).getKey()
                           .getData();
      ...
      try{
         ...
         if(itemlist.getResult().next()){
            ...
            do{
               ...
               Item i = new Item(itemlist.getResult().getInt("itemid"),
                     itemlist.getResult().getByte("data"), itemlist.getResult()
                           .getShort("durability"));
               if(i.isTool()){
         ...
         if(!empty){
            ...
            if(valid){
               // Header with amount of pages
               sender.sendMessage(ChatColor.BLUE + "===" + ChatColor.GOLD
                     + current + ChatColor.BLUE + "===" + ChatColor.GREEN
                     + "Page: "
               ...
               for(int i = ((Karma.page.get(sender.getName()).intValue()) * RootConfig
                  ...
                  if(i < array.length){
                     ...
                     sb.append(ChatColor.WHITE + " Amount: " + ChatColor.GOLD
