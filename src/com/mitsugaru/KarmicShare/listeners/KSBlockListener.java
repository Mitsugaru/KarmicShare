   ...
   public void onSignChange(final SignChangeEvent event) {
      ...
      if(!ChatColor.stripColor(event.getLine(2)).equals("")) {
         ...
         if(RootConfig.getBoolean(ConfigNode.CHESTS) && plugin.useChest()) {
            final Material down = event.getBlock().getRelative(BlockFace.DOWN)
                  .getType();
            // Thanks to Wolvereness for the following code
            if(down.equals(Material.CHEST) || down.equals(Material.ENDER_CHEST)) {
               // Reformat sign
               ...
               event.setLine(3, "1");
               event.getPlayer().sendMessage(
                     ChatColor.YELLOW + KarmicShare.TAG + " No chest found!");
            }
         } else {
            event.getPlayer().sendMessage(
                  ChatColor.RED + KarmicShare.TAG + " Chests access disabled");
            // Cancel event
            event.setCancelled(true);
         }
      }
      ...
      if(material.equals(Material.SIGN) || material.equals(Material.WALL_SIGN)
         ...
         if(block.getRelative(BlockFace.UP).getType() == Material.WALL_SIGN) {
            ...
            if(chest.attachedBlock().getRelative(BlockFace.UP).getType()
                  .equals(Material.WALL_SIGN)) {
               sign = (Sign) chest.attachedBlock().getRelative(BlockFace.UP)
                     .getState();
               for(String s : sign.getLines()) {
                  if(ChatColor.stripColor(s).equalsIgnoreCase(KarmicShare.TAG)) {
                     // Sign already exists
         ...
         if(has) {
            ...
            if(RootConfig.getStringList(ConfigNode.DISABLED_WORLDS).contains(
                  world)) {
               event.getPlayer().sendMessage(
                     ChatColor.RED + KarmicShare.TAG
                           + " KarmicShare access disabled for this world.");
