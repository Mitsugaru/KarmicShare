...
public class ItemCommands{
   ...
   static void takeItem(CommandSender sender, String[] args){
      ...
      if(!PermissionHandler.has(sender, PermissionNode.COMMANDS_TAKE)){
         sender.sendMessage(ChatColor.RED + KarmicShare.TAG
               + " Lack permission: " + PermissionNode.COMMANDS_TAKE.getNode());
         return;
      }
      ...
      if(temp.isTool()){
         ...
         try{
            ...
            if(toolRS.getResult().next()){
               do{
                  // Generate item
                  ItemStack toolItem = new ItemStack(itemid, toolRS.getResult()
                        .getInt("amount"), toolRS.getResult().getShort("data"));
                  String enchant = toolRS.getResult().getString("enchantments");
                  if(!toolRS.getResult().wasNull()){
                     ...
                     for(int i = 0; i < cut.length; i++){
                        try{
                           ...
                           toolItem.addUnsafeEnchantment(e.getEnchantment(),
         ...
         if(finalAmount > 0){
            ...
            final HashMap<Integer, ItemStack> residual = player.getInventory()
                  .addItem(item);
            if(residual.size() != 0){
               // Add back extra
               finalAmount -= residual.size();
               ...
               try{
                  int currentKarma = Karma.getPlayerKarma(player.getName());
   ...
   static void giveItem(CommandSender sender, String[] args){
      ...
      if(!PermissionHandler.has(sender, PermissionNode.COMMANDS_GIVE)){
         sender.sendMessage(ChatColor.RED + KarmicShare.TAG
               + " Lack permission: " + PermissionNode.COMMANDS_GIVE.getNode());
         return;
      }
      // Grab group
      String group = Karma.selectedGroup.get(sender.getName());
   ...
   static void inspectItem(CommandSender sender, String[] args){
      ...
      buf.append("Info: Name: " + ChatColor.AQUA + item.name + ChatColor.GREEN
            + " ID: " + ChatColor.LIGHT_PURPLE + itemid + ChatColor.GREEN
            + " Amount:" + ChatColor.GOLD + quantity + ChatColor.GREEN
            + " Data: " + ChatColor.LIGHT_PURPLE + item.getData()
            + ChatColor.GREEN + " Damage: " + ChatColor.LIGHT_PURPLE
            + items.getDurability() + ChatColor.GREEN + " Tool: "
            + ChatColor.GRAY + item.isTool() + ChatColor.GREEN + " Potion: "
      ...
      if(!RootConfig.getBoolean(ConfigNode.KARMA_DISABLED)){
         if(RootConfig.getBoolean(ConfigNode.KARMA_STATIC)){
            ...
            buf.append(ChatColor.GREEN
                  ...
                  + ChatColor.YELLOW
                  + ""
                  + (RootConfig.getInt(ConfigNode.KARMA_CHANGE_GIVE) * quantity)
                  + " : "
                  + (RootConfig.getInt(ConfigNode.KARMA_CHANGE_TAKE) & quantity));
         }else{
            // Check if given item has a multiplier
            Item[] karmaList = KarmaConfig.VALUES.keySet().toArray(new Item[0]);
            ...
            if(hasKarma){
               try{
                  buf.append(ChatColor.GREEN + " Multiplier: "
                        + ChatColor.YELLOW + KarmaConfig.VALUES.get(item).getGiveValue() + " : " + KarmaConfig.VALUES.get(item).getTakeValue());
                  buf.append(ChatColor.GREEN + " Total Karma: "
                        + ChatColor.YELLOW + ""
                        + (KarmaConfig.VALUES.get(item).getGiveValue() * quantity) + " : " +  (KarmaConfig.VALUES.get(item).getTakeValue() * quantity));
